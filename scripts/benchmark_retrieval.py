...
async def git_revision(path: Path) -> str:
    process = await asyncio.create_subprocess_exec(
        "git", "-C", str(path), "rev-parse", "HEAD", stdout=asyncio.subprocess.PIPE
    )
    stdout, _ = await process.communicate()
    if process.returncode != 0:
        raise RuntimeError(f"cannot resolve git revision for {path}")
...
async def run(dataset_path, output_path, top_k):
    ...
    try:
        ...
        metrics = {
            ...
            "recall_at_k": statistics.mean(row["recall"] for row in rows),
        ...
        payload = {
            "metadata": {
                ...
                "incident_repo_commit": await git_revision(Path.cwd()),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("benchmarks/latest_retrieval_results.json")
    )
