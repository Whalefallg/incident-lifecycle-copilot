...
class McpNotConfigured(McpError): pass
class McpStartupError(McpError): pass
class McpProtocolError(McpError): pass
class McpCapabilityError(McpError): pass
class McpTimeoutError(McpError): pass
class McpProcessExited(McpError): pass
...
class McpRagClient:
            ...
            try:
                self._process = await asyncio.create_subprocess_exec(
                    *command, stdin=asyncio.subprocess.PIPE, stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE, cwd=str(path), env=env,
                )
                self._stderr_task = asyncio.create_task(self._drain_stderr(self._process))
                initialized = await self._request(
                    "initialize",
                    {"protocolVersion": "2024-11-05", "clientInfo": {"name": "incident-lifecycle-copilot", "version": "1.0"}},
                    timeout=self.settings.startup_timeout_seconds,
                )
                await self._notify("notifications/initialized", {})
                tools_result = await self._request("tools/list", {}, timeout=self.settings.startup_timeout_seconds)
    ...
    async def query(self, query: str, *, top_k: int = 10, collection: str | None = None) -> list[RetrievalResult]:
        if not 1 <= top_k <= 50:
            raise ValueError("top_k must be in range 1..50")
        arguments = {"query": query, "top_k": top_k, "collection": collection or self.settings.collection}
        for attempt in range(self.settings.max_retries + 1):
            try:
                self._require_started()
                response = await self._request("tools/call", {"name": self.REQUIRED_TOOL, "arguments": arguments})
    ...
    async def _request(self, method: str, params: dict[str, Any], *, timeout: float | None = None) -> dict[str, Any]:
        async with self._lock:
            ...
            try:
                ...
                line = await asyncio.wait_for(process.stdout.readline(), timeout=timeout or self.settings.request_timeout_seconds)
    ...
    @classmethod
    def _validate_capabilities(cls, initialized: dict[str, Any], tools_result: dict[str, Any]) -> McpCapabilities:
        ...
        return McpCapabilities(str(server["name"]), str(server["version"]), tuple(str(t.get("name")) for t in tools))
